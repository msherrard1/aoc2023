...
def check_card_matches(card_content):
...
def get_matches(cards):
    ...
    for card_id, card_content in cards.items():
        card_id_to_matches[card_id] = check_card_matches(card_content)
    
    return card_id_to_matches

def process_cards(cards):
    card_id_to_num_cards = { }
    for card_id in cards.keys():
        card_id_to_num_cards[card_id] = 1

    card_id_to_matches = get_matches(cards)
    
    for card_id, num_cards in card_id_to_num_cards.items():
        matches = card_id_to_matches[card_id]
        for card_run in range(num_cards):
            for i in range(card_id+1,card_id+matches+1):
                card_id_to_num_cards[i] += 1
    
    return card_id_to_num_cards
...
# Card 6: 31 18 13 56 72 | 74 77 10 23 35 67 36 11"""
    ...
    running_sum = 0
    for card_id, card_count in card_counts.items():
        running_sum += card_count
    print(running_sum)